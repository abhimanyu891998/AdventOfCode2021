...
bool smallHasBeenVisitedTwice = false;
string smallVisitedTwice = "";

void addEdge(map<string, vector<string>> &graph, const string &u, const string &v) {
        graph[u].push_back(v);
        graph[v].push_back(u);
}

void dfs(map<string, vector<string>> &graph, const string &s, map<string, bool> visited, int &ctr,  const bool &part2) {
        if(s == "end") {
            ctr++;
            return;
        }
